...
#m = nb_breakpoints
function methode_2(functions,m::Vector{Int64},bornes::Vector{Vector{Float64}})
    ...
    + sum(d[i,l] for l in 1:(k-1)))) for k in convexe[i]) + sum(((s[i][k] - s[i][k-1])*(2*x[i]-2*z[i,k] +2*breakpoints[i][k]*u[i,k] - 2*breakpoints[i][k] ))
