...
function methode_2_review(x::Float64,m::Int64,bornes::Vector{Float64})
    ...
    if !isempty(convexe)
        L_f_x += 2*sum(((s[k] - s[k-1])*(x - breakpoints[k] + sum(value(d[l]) for l in 1:(k-1)))) for k in convexe )
    end
    if !isempty(concave)    
...
f(x) = x->x^3-2*x^2-x+1
fpoints = map(f,[k for k in 1:0.01:2])

plot([k for k in 1:0.01:2],[points_approches,fpoints])