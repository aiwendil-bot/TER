...
function recup_glpk()
    ...
    nb_breakpoints::Vector{Int64} = [k for k in 319:329]
    ...
    open("focus/glpk/output_temps.txt") do file
    ...
    writedlm("focus/glpk/temps.txt", temps)
    ...
    ptemps = plot(nb_breakpoints, temps, title="Min sum x_i, sum x_i^2 <= 1", label="n=20",xlabel="nb_breakpoints", ylabel="temps execution")
    #savefig(ptempslog, "glpk_tempsloglog.png")
    savefig(ptemps, "focus/glpk/focus_glpk_temps.png")
